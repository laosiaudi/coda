...
int fsobj::Fakeify()
{
    ...
    if (FID_VolEQ(&fid, &fakefid)) {
        if (FID_EQ(&fid, &fakefid)) { /* actual root directory? */
            ...
            list_for_each(p, REALMDB->realms) {
                ...
                LOG(0, ("fsobj::Fakeify: created fake codaroot entry for %s\n",
                            realm->Name()));
            }

            fakefid.Vnode = 0xfffffff9;
            fakefid.Unique = 0x1;
            dir_Create(".pioctl", &fakefid);
            LOG(0, ("fsobj::Fakeify: created fake codaroot directory\n"));
        } else if (fid.Vnode == 0xfffffff9) {
            stat.Mode = 0644;
            stat.LinkCount = 1;
            stat.VnodeType = Directory;
            stat.Length = 4;
            ...
            data.file->SetValidData(0);

            LOG(0, ("fsobj::Fakeify: created fake listcache file\n"));
        } else {
    ...
    if (FID_VolEQ(&fid, &fakefid)) {
        if (!FID_IsFakeRoot(MakeViceFid(&fid))) {
            ...
            LOG(0, ("fsobj::Fakeify: new entry (localhost, %s)\n", FID_(&fakefid)));
            ...
            for (int i = 0; i < VSG_MEMBERS; i++) {
                ...
                LOG(0, ("fsobj::Fakeify: new entry (%s, %s)\n", s->name, FID_(&fakefid)));
            }
        } else {
...
void fsobj::CreateCFSFakeFile() {
    char* filenames[8] = {"listcache", "checkservers", "checkvolumes",
        "clearpriorities", "flushcache", "flushvolume", "listacl", "lsmount"};
    uint32_t start = 0xfffffff8;
    uint32_t idx = 0;
    for (; idx < 16; idx += 2) {
        VenusFid fakefid1;
        fakefid1.Vnode = start - idx;
        LOG(0, ("fsobj::Fakeify: creating fake file, vnode is %d\n", fakefid1.Vnode));
        fakefid1.Unique=1;
        dir_Create(filenames[idx / 2], &fakefid1);

        //create metadata file for writing parameters
        VenusFid fakefid2;
        fakefid2.Vnode = start - idx - 1;
        LOG(0, ("fsobj::Fakeify: creating fake file, vnode is %d\n", fakefid2.Vnode));
        fakefid2.Unique=1;
        char metadata[50];
        strcpy(metadata, ".");
        strcat(metadata, filenames[idx / 2]);
        dir_Create(metadata, &fakefid2);

    }
    LOG(0, ("fsobj::Fakeify: created fake .pioctl directory and files\n"));
