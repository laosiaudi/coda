...
{
    LOG(0, ("vproc::open: fid = %s , flags = %x\n",
                FID_(&cp->c_fid), flags));
    ...
    for (;;) {
        ...
        /* Do the operation. */
        if (cp->c_fid.Vnode <= 0xfffffff8 && cp->c_fid.Vnode >= 0xffffffe8) {
            ...
            //f->data.file->SetValidData(total);
            LOG(0, ("\n------------------------- 0xffffffff8, pioctl finish initialzation"));
...
{
    LOG(0, ("vproc::ioctl(%d): fid = %s, com = %s\n",
        ...
        struct venus_cnode *cp, int flags)
{
    ...
    LOG(0, ("vproc::lookup: fid = %s, name = %s, nc = %x\n",
